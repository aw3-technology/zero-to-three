# Chapter 43: Hiring and Team Building in Web3 and AI
...
```mermaid
flowchart TD
    A[Identify Role Need] --> B{Does role require Web3/AI domain expertise?}
    B -->|Yes| C[Source from domain-specific channels: open source, conferences, communities]
    B -->|No| D[Source from traditional channels with domain onboarding plan]
    C --> E{Is role technical or community-facing?}
    E -->|Technical| F[Evaluate: open-source contributions, published work, scenario-based assessments]
    E -->|Community| G[Evaluate: communities built, engagement quality, conflict resolution skills]
    D --> H[Evaluate: adaptability, learning velocity, culture fit]
    F --> I{Full-time or contributor model?}
    G --> I
    H --> I
    I -->|Full-time| J[Design compensation: base + tokens/equity + non-financial incentives]
    I -->|Contributor| K[Design: bounties, grants, or open-source contribution framework]
    J --> L[Structure: co-founder prenup or employment agreement with vesting]
    K --> L
```
