# Chapter 13: Market Validation

> *Last Updated: March 2026*

> **Skim This Chapter**
> - Market validation in Web3 and AI requires testing psychological adoption factors, not just technical feasibility; use stage-gated validation ladders with explicit pass/fail criteria and timeboxes to eliminate false positives before committing resources.
> - Output: a lean validation plan with specific signal thresholds, trust-barrier checklists, and ready-to-use test templates for your market segment.
...
> **The validation rule:** If you cannot describe your first ten users by name, you are not validating---you are guessing.
...
The following diagram illustrates the core market validation process flow that underpins all the stage-gated ladders below. Each cycle tightens the feedback loop between hypothesis and evidence, progressively eliminating false positives before you commit significant resources.

```mermaid
flowchart TD
    A[Form Core Hypothesis] --> B[Identify First 10\nTarget Users by Name]
    B --> C[Design Minimum\nViable Test]
    C --> D[Run Test with\nExplicit Pass/Fail Criteria]
    D --> E{Did signals meet\nthreshold?}
    E -->|Yes: Pass| F[Advance to\nNext Stage]
    E -->|No: Below threshold\nbut close| G[Iterate: Refine\nmessaging, channel,\nor pricing]
    E -->|No: Fail Fast\ncriteria met| H[Pivot: Revisit\nproblem space or\nsolution approach]
    G --> D
    H --> A
    F --> I{Final stage\ncomplete?}
    I -->|No| J[Increase commitment:\nmore users, real payment,\non-chain traction]
    I -->|Yes| K[Validated: Commit\nresources to build]
    J --> C
```
...
### Practical Market Sizing for Web3/AI Ventures

When presenting market sizing to investors or validating your opportunity internally, avoid the common trap of citing only TAM (the "trillion-dollar market" slide). Sophisticated investors—and honest self-assessment—require all three levels:

**TAM (Total Addressable Market)**: The entire revenue opportunity if you achieved 100% market share. Useful for context but misleading in isolation. Every startup in every sector can find a TAM that sounds impressive.

**SAM (Serviceable Addressable Market)**: The portion of TAM you can realistically reach with your current product, business model, and go-to-market strategy. This is the number that matters for your Series A pitch.

**SOM (Serviceable Obtainable Market)**: What you can realistically capture in the next 3–5 years given competition, execution risk, and resource constraints. This is the number that should inform your financial model.

**Red flags in market sizing** (what experienced investors see through):
- Citing only TAM without SAM/SOM breakdown
- Using top-down sizing ("X% of a $Y trillion market") without bottom-up validation
- Conflating adjacent markets to inflate numbers
- Ignoring substitutes and competitive alternatives
- Presenting speculative AI/Web3 convergence markets without acknowledging their nascent status

**Bottom-up validation approach** (what builds credibility):
1. Count your actual potential customers (named accounts or identifiable segments)
2. Estimate realistic spend per customer based on comparable solutions they use today
3. Apply conservative conversion and market share assumptions
4. Cross-check against top-down estimates for reasonableness
...
- **The Mom Test** by Rob Fitzpatrick — A practical guide to conducting customer conversations that yield honest, actionable feedback rather than false validation.
- **Testing Business Ideas** by David J. Bland and Alexander Osterwalder — A comprehensive library of 44 experiments for rapidly testing business hypotheses, organized by cost and evidence strength.
- **Inspired: How to Create Tech Products Customers Love** by Marty Cagan — Essential reading on product discovery techniques that separate genuine demand signals from noise.
- **The Right It** by Alberto Savoia — Introduces "pretotyping" methods for testing market demand before building anything, with data-driven validation frameworks.
...
**Previously:** [Chapter 10: Idea Development](../part-01-understanding-the-new-terrain/ch10-idea-development.md) — Explored how to build competitive moats in open systems through data network effects, community advantages, and defensible positioning.

**Next:** [Chapter 14: Founder's Psychology](ch14-founders-psychology.md) — Examines the inner operating system of the founder and how psychological architecture shapes every decision in a venture.
